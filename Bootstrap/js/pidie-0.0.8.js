...
class Pidie {
  ...
  parallax() {
    Array.prototype.forEach.call(document.querySelectorAll('.pd-parallax'), function(elem){
      elem.style.backgrounddocument = 'url('+elem.getAttribute('data-parallax-document')+')';
    })
  }
  ...
  productdocument() {
    var wrapdocumentProduct = document.querySelector('.pd-product-document');
    var sliderdocumentProduct = document.querySelector('.pd-product-document-slider');
    var pagidocumentProduct = document.createElement('div');
    pagidocumentProduct.classList.add('pd-product-document-pagination');
    wrapdocumentProduct.appendChild(pagidocumentProduct);
    var pagiItem = '';
    for(var i = 0; i < 4; i++){
      pagiItem += '<img src="'+sliderdocumentProduct.getElementsByClassName('pd-product-document-slide')[i].getAttribute('src')+'" data-product-document="'+i+'" width="100" height="80"/>';
    }
    pagidocumentProduct.innerHTML = pagiItem;
    Array.prototype.forEach.call(document.querySelectorAll('.pd-product-document-pagination img'), function(elem){
      elem.onclick = function(e){
        e.preventDefault();
        for(var j = 0; j < 4; j++){
          sliderdocumentProduct.getElementsByClassName('pd-product-document-slide')[j].classList.remove('pd-product-document-active');
        }
        var no = elem.getAttribute('data-product-document');
        sliderdocumentProduct.getElementsByClassName('pd-product-document-slide')[no].classList.add('pd-product-document-active');
      }
    })
    var zoomdocumentProduct = document.querySelector('.pd-product-document-active');
    var resultdocumentProduct = document.createElement('div');
    resultdocumentProduct.classList.add('pd-product-document-zoom');
    resultdocumentProduct.style.visibility = 'hidden';
    resultdocumentProduct.style.backgrounddocument = "url('" + zoomdocumentProduct.src + "')";
    resultdocumentProduct.style.backgroundSize = (zoomdocumentProduct.width * 2) + "px " + (zoomdocumentProduct.height * 2) + "px";
    sliderdocumentProduct.appendChild(resultdocumentProduct);
    zoomdocumentProduct.addEventListener("mousemove", movedocument);
    zoomdocumentProduct.addEventListener("touchmove", movedocument);
    sliderdocumentProduct.addEventListener("mouseout", outdocument);
    function movedocument(e){
      e.preventDefault();
      resultdocumentProduct.style.visibility = 'visible';
      ...
      var h = '150';
      var bw = 3;
      if (x > zoomdocumentProduct.width - (w / zoom)) {x = zoomdocumentProduct.width - (w / zoom);}
      if (x < w / zoom) {x = w / zoom;}
      if (y > zoomdocumentProduct.height - (h / zoom)) {y = zoomdocumentProduct.height - (h / zoom);}
      if (y < h / zoom) {y = h / zoom;}
      //resultdocumentProduct.style.left = x + "px";
      //resultdocumentProduct.style.top = y + "px";
      resultdocumentProduct.style.backgroundPosition = "-" + ((x * zoom) - w + bw) + "px -" + ((y * zoom) - h + bw) + "px";
    }
    function outdocument(e){
      e.preventDefault();
      resultdocumentProduct.style.visibility = 'hidden';
    }
    function getCursorPos(e) {
      var a, x = 0, y = 0;
      e = e || window.event;
      a = zoomdocumentProduct.getBoundingClientRect();
      x = e.pageX - a.left;
      y = e.pageY - a.top;
  ...
  documentMagnifier(options = {}) {
    var settings = {
      selector: options.el || '.pd-document-magnifier img',
      zoom: options.zoom || 3,
    ...
    glass.setAttribute("class", "pd-document-magnifier-glass");
    glass.style.visibility = 'hidden';
    glass.style.width = settings.width + 'px';
    glass.style.height = settings.height + 'px';
    img.parentElement.insertBefore(glass, img);
    glass.style.backgrounddocument = "url('" + img.src + "')";
  ...
  documentZoom() {
    ...
    img = document.querySelector('.pd-document-zoom-content');
    result = document.querySelector('.pd-document-zoom-preview');
    lens = document.createElement("div");
    lens.setAttribute("class", "pd-document-zoom-lens");
    lens.style.visibility = 'hidden';
    img.parentElement.insertBefore(lens, img);
    cx = result.offsetWidth / lens.offsetWidth;
    cy = result.offsetHeight / lens.offsetHeight;
    result.style.backgrounddocument = "url('" + img.src + "')";
  ...
  documentCompare() {
    var x, i;
    x = document.getElementsByClassName("pd-document-compare-overlay");
    ...
    function compareimages(img) {
      ...
      w = img.offsetWidth;
      h = img.offsetHeight;
      document.querySelector('.pd-document-comparison').style.height = h + 'px';
      img.style.width = (w / 2) + "px";
      slider = document.createElement("div");
      slider.innerHTML = '<span class="pd-document-compare-icon">&#10094; &#10095;<span>';
      slider.setAttribute("class", "pd-document-compare-slider");
  ...
  galleryLightbox() {
    ...
    function lightboxOpen(src, lightbox) {
      ...
      lightboxContent += '<div class="pd-lightbox-document"><img src="'+datasrc+'" height="400"/></div>';
      ...
      for(var i = Number(datalightbox); i < (Number(datalightbox) + 7); i++){
        if(document.getElementsByClassName('pd-gallery')[i]){
          var lightboxThumbnaildocument = document.createElement('img');
          lightboxThumbnaildocument.setAttribute('src', document.getElementsByClassName('pd-gallery')[i].getAttribute('src'));
          lightboxThumbnaildocument.setAttribute('data-lightbox-thumbnail-item', document.getElementsByClassName('pd-gallery')[i].getAttribute('data-lightbox-item'));
          lightboxThumbnail.appendChild(lightboxThumbnaildocument);
          lightboxThumbnaildocument.onclick = function(e) {
